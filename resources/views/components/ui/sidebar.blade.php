                    ...
                    <li>
                        <details {{ $isActive ? 'open' : '' }}>
                            <summary class="flex items-center gap-2">
                                @if(!empty($item['icon']))
                                    <x-daisy::ui.icon :name="$item['icon']" :prefix="$iconPrefix" size="md" />
                                @endif
                                ...
                                @foreach($item['children'] as $child)
                                    <li>
                                        <a href="{{ $child['href'] ?? '#' }}" class="flex items-center gap-2 {{ !empty($child['active']) ? 'menu-active' : '' }}">
                                            @if(!empty($child['icon']))
                                                <x-daisy::ui.icon :name="$child['icon']" :prefix="$iconPrefix" size="md" />
                                            @endif
        ...
        @if($showToggle)
            <button type="button" class="btn btn-ghost btn-sm w-full justify-between sidebar-toggle">
                ...
                @if($effectiveCollapsed)
                    <x-daisy::ui.icon name="chevron-double-right" :prefix="$iconPrefix" size="sm" />
                @else
                    <x-daisy::ui.icon name="chevron-double-left" :prefix="$iconPrefix" size="sm" />
                @endif
            </button>
        @endif
    </div>
