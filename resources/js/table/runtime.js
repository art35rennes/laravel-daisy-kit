...
import {
  mutationRequest as sendMutationRequest,
  isSameOrigin,
...
import { resolveTableRoot } from './dom.js';
  ...
  return columns
    ...
    .map((column) => {
      ...
      return {
        ...
        trusted: ['trusted-html', 'blade'].includes(cell.renderer),
        cell,
        editor: {
          type: editorType,
...
function normalizeEditableConfig(raw = {}, columns = []) {
  ...
  const defaultEditableKeys = columns
    .filter((column) => !column.trusted && !['actions', 'link', 'resource-link'].includes(column.type))
...
function normalizeEndpoint(endpoint) {
  ...
  if (isPlainObject(endpoint) && typeof endpoint.url === 'string' && endpoint.url !== '') {
    return {
      ...
      method: endpoint.method ? normalizeHttpMethod(endpoint.method) : undefined,
    };
  }
...
function normalizeConfig(raw = {}) {
  ...
  const config = {
    ...
    mode,
    method: normalizeHttpMethod(raw.method, DEFAULT_METHOD),
    serverAdapter,
    persistState,
    stateKey: typeof raw.stateKey === 'string' && raw.stateKey !== '' ? raw.stateKey : null,
    persistStateFields: normalizePersistedStateFields(raw.persistStateFields),
...
function buildServerRequest(config, state) {
  ...
  if (config.method === 'GET') {
    ...
    if (csrfToken && isSameOrigin(endpoint) && !headers.has('X-CSRF-TOKEN')) {
    ...
    th.innerHTML = context.config.selection.mode === 'multiple'
      ? `
          <input
            ...
            ${context.selectionReadOnly ? 'disabled' : ''}
          >
        `
...
async function commitCreate(context) {
  ...
  const rowKey = getStableRowKey(context.config);
  const draftId = context.editing.rowId;
  const controller = typeof AbortController !== 'undefined' ? new AbortController() : null;

  if (controller) {
    context.mutationControllers.add(controller);
  }
  ...
  try {
    let body = { row: { ...values, [rowKey]: values[rowKey] ?? draftId } };

    if (create.strategy === 'remote') {
      const response = await sendMutationRequest(create.endpoint, create.method, { values }, {
    ...
    const row = requireMutationRow(body, rowKey);
    ...
    if (context.destroyed || error?.name === 'AbortError') {
      return null;
    }
    ...
    if (controller) {
      context.mutationControllers.delete(controller);
    }
  }
}
...
async function commitEdit(context) {
  ...
  const payload = buildUpdatePayload(context.editing);
  const controller = typeof AbortController !== 'undefined' ? new AbortController() : null;
  ...
  try {
    let body = { row: { ...row, ...payload.dirty } };

    if (context.config.editable.update.strategy === 'remote') {
      const response = await sendMutationRequest(context.config.editable.update.endpoint, context.config.editable.update.method, payload, {
        rowId,
        signal: controller?.signal,
      });
      body = await response.json().catch(() => ({}));
    ...
    const nextRow = requireMutationRow(body, getStableRowKey(context.config));
...
function collectClientRowIds(context, rows = context.rows) {
...
function setClientRows(context, rows) {
  ...
  validateRows(rows, getStableRowKey(context.config), context.config.subRowsKey, 'setRows');
  context.rows = cloneRows(rows);
...
function upsertClientRows(context, rows) {
  requireClientDataContext(context);
  const rowKey = getStableRowKey(context.config);
  validateRows(rows, rowKey, context.config.subRowsKey, 'upsertRows');
  ...
  rows.forEach((row) => {
    ...
    rowsById.set(rowId, cloneRows([row])[0]);
  });

  const existingRowIds = new Set();
  const nextRows = context.rows.map((row) => {
    const rowId = getRowSelectionId(context.config, row);
  ...
  validateRows(nextRows, rowKey, context.config.subRowsKey, 'upsertRows');
  context.rows = nextRows;
...
function removeClientRows(context, rowIds) {
  ...
  context.rows = context.rows.filter((row) => {
    const rowId = getRowSelectionId(context.config, row);
...
function getPersistedStateKey(context) {
  const key = context.config.stateKey || context.root.id;

  return key ? `daisy-table:${key}` : null;
}
...
function parseStateFromUrl(config) {
  ...
  return readStateFromUrl(config);
}
...
function parseStateFromLocalStorage(context) {
  ...
  try {
    const key = getPersistedStateKey(context);
...
function persistState(context) {
  ...
  if (context.config.persistState === 'url') {
    try {
      writeStateToUrl(context.config, context.root, context.state);
    } catch (_) {}
    return;
  }
  ...
  try {
    ...
    if (key) {
      window.localStorage.setItem(key, serializePersistedState(context.state, context.config.persistStateFields));
    }
  } catch (_) {}
}
...
function mergeState(baseState, overrideState = {}, config, mergePartialObjects = false) {
  ...
  if (isPlainObject(overrideState.rowSelection)) {
    ...
    if (config.selection.enabled && !isPlainObject(overrideState.selection)) {
      ...
      nextState.selection = normalizeSelectionState({
        selectedIds: config.selection.mode === 'single' ? selectedIds.slice(-1) : selectedIds,
      });
    }
  }
...
async function refreshTable(context) {
  ...
  try {
    ...
    if (context.destroyed || refreshId !== context.refreshId) {
      return;
    }

    if (context.config.rowKey) {
      validateRows(response.rows, context.config.rowKey, context.config.subRowsKey, 'serverResponse');
    }
...
function attachEvents(context) {
  ...
  context.root.addEventListener('click', (event) => {
    ...
    const rowActionButton = event.target instanceof Element ? event.target.closest('[data-table-row-action]') : null;

    if (rowActionButton instanceof HTMLButtonElement) {
      if (rowActionButton.disabled) {
        return;
      }

      const rowId = rowActionButton.dataset.tableRowId || '';
      const columnId = rowActionButton.dataset.tableColumnId || '';
      const row = context.table?.getRow(rowId, true)?.original
        ?? getVisibleRowById(context, rowId)?.original
        ?? null;
      const column = context.table?.getColumn(columnId) ?? null;

      context.root.dispatchEvent(new CustomEvent('daisy:table-row-action', {
        bubbles: true,
        detail: {
          action: rowActionButton.dataset.tableRowAction,
          rowId,
          row: row ? cloneState(row) : null,
          column,
          table: context.table,
        },
      }));
...
function destroyTable(root) {
  ...
  context.mutationControllers.forEach((controller) => controller.abort());
  context.mutationControllers.clear();
...
async function initTable(root) {
  ...
  const rawConfig = parseConfig(container);

  assertContractVersion(rawConfig);

  const config = normalizeConfig(rawConfig);

  if (config.rowKey && config.rows.length > 0) {
    validateRows(config.rows, config.rowKey, config.subRowsKey, 'initialRows');
  }
  ...
  const context = {
    ...
    creating: null,
    editing: null,
    selectionReadOnly: config.selection.readOnly,
...
function tableApi(idOrRoot) {
  ...
  return {
    ...
    getRows() {
      ...
      return context ? snapshotContext(context).rows : [];
    },
    getState() {
      const context = root.__daisyTableContext ?? null;

      return context ? cloneState(context.state) : null;
    },
    getTanStackTable() {
      return root.__daisyTableContext?.table ?? null;
    },
    snapshot() {
      const context = root.__daisyTableContext ?? null;

      return context ? snapshotContext(context) : null;
    },
    async setRows(rows = []) {
      const context = await initTable(root);
    ...
    async setSelectionReadOnly(readOnly = false) {
      ...
      context.selectionReadOnly = readOnly === true;
      context.root.dataset.tableSelectionReadonly = context.selectionReadOnly ? 'true' : 'false';
      context.root.setAttribute('aria-disabled', context.selectionReadOnly ? 'true' : 'false');
      renderTable(context);
