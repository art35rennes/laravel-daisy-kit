# V6 product contract matrix

This document is the sole parity oracle for the v6 release. “Parity” means
...
| Host code reads and changes selection or expansion through a stable facade without private DOM access. | v6 integrator-facade contract. | Vitest `getValue`/`setValue`/`clear`/`expand`/`collapse`/`focus` and canonical `change` payload tests. |
...
| Host code reads, replaces, selects, arranges and traverses history through one facade that survives structural remounts. | v6 integrator-facade contract. | Vitest facade identity, detached snapshots, commands and event-payload tests. |
...
| Host code controls preview visibility, expansion, zoom and reload without accessing the sandbox or renderer. | v6 integrator-facade contract. | Vitest facade state/commands, reload cancellation and event-payload tests. |
...
| Map keeps strict parent-page style CSP; the only documented style-attribute exceptions are Signature and Transfer List. | ADR-002. | CSP violation/console test with configured directives. |
