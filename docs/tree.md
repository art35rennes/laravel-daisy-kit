...
ids outside the visible search or beneath collapsed branches. "Select loaded results" selects only
visible loaded leaves; it does not select collapsed descendants or fetch the full tree.
...
the query until Search, Enter or `applySearch()`. Clear search restores pre-search expansion.

`searchSource` receives GET requests with `searchParam="query"`. Search and lazy endpoints return
`{ "items": [...] }`. Search results include ancestor paths, with `source` retained for partial
...
`clearSearch`, `expandAll`, `collapseAll`, `selectVisible`. Asynchronous commands return
`Promise<boolean>`: `expand`, `expandPath`, `applySearch`, `reloadBranch`.
...
under strict CSP. Use `theme=dark` for dark mode. No editor, drag-and-drop, pagination or
